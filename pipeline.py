...
def find_closest_matches(query: str, candidates: List[str], max_matches: int = 5) -> List[str]:
    """Find closest matching database IDs using substring and similarity."""
    query_lower = query.lower()
    
    substring_matches = [c for c in candidates if query_lower in c.lower()]
    if substring_matches:
        return substring_matches[:max_matches]
    
    from difflib import SequenceMatcher
    scored = [(c, SequenceMatcher(None, query_lower, c.lower()).ratio()) for c in candidates]
    scored.sort(key=lambda x: x[1], reverse=True)
    return [c for c, score in scored[:max_matches] if score > 0.4]


def validate_db_id(db_id: Optional[str], schema_lookup: Dict[str, Any]) -> str:
    """Validate database ID exists in schema lookup. Returns str. Suggests closest matches on failure."""
    ...
    if db_id_str and db_id_str not in schema_lookup:
        all_dbs = list(schema_lookup.keys())
        suggestions = find_closest_matches(db_id_str, all_dbs, max_matches=5)
        
        if suggestions:
            suggest_text = ", ".join(suggestions)
            raise ValueError(f"❌ Database '{db_id_str}' not found!\n   Did you mean: {suggest_text}?")
        else:
            available = all_dbs[:5]
            raise ValueError(f"❌ Database '{db_id_str}' not found!\n   Available: {available}")
...
class DatabaseDiscovery:
    ...
    def search_databases(self, keyword: str, include_non_existent: bool = False) -> List[str]:
        ...
        for db_id, info in self.db_cache.items():
            if not info['exists'] and not include_non_existent:
                continue
    ...
    def get_best_matching_db(self, question: str) -> Optional[str]:
        """Auto-detect best matching DB based on question keywords."""
        stop_words = {'how', 'many', 'what', 'where', 'when', 'why', 'which', 'is', 'are', 'the', 'a', 'an', 'to', 'from', 'and', 'or'}
        words = [w.lower() for w in question.split() if w.lower() not in stop_words and len(w) > 2]
        
        if not words:
            return None
        
        all_matches: Dict[str, int] = defaultdict(int)
        for word in words:
            matches = self.search_databases(word)
            for db in matches:
                all_matches[db] += 1
        ...
        best_db = max(all_matches.items(), key=lambda x: x[1])[0]
        return best_db
...
def run_agentic_pipeline(
    ...
    try:
        if db_id is not None:
            ...
            best_db = db_discovery.get_best_matching_db(question_clean)
            if best_db:
                db_ids = [best_db]
                print(f"  ✓ Selected: {best_db}")
            else:
                raise ValueError("❌ Could not auto-detect database! Please specify db_id or db_ids.")
            db_ids_clean = validate_db_list(db_ids, schema_lookup)
        else:
            raise ValueError("❌ Please specify db_id, db_ids, or enable auto_detect_db=True")
    except ValueError as e:
        print(f"\n{e}")
        raise
    
    if 'db_ids_clean' not in locals():
        db_ids_clean = validate_db_list(db_ids, schema_lookup)
    ...
    while True:
        try:
            ...
            if not cmd:
                ...
                print("\nOptions:")
                print("  1. Specify database ID manually")
                print("  2. Auto-detect database from question")
                ...
                if choice == "1":
                    ...
                    if not db_id:
                        ...
                        continue
                    try:
                        run_agentic_pipeline(db_id=db_id, question=question, use_llm=LLM_AVAILABLE, auto_detect_db=False)
                    except Exception as e:
                        print(f"Error: {e}")
                else:  # choice == "2" or default
                    try:
                        run_agentic_pipeline(question=question, use_llm=LLM_AVAILABLE, auto_detect_db=True)
                    except Exception as e:
                        print(f"Error: {e}")
            else:
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("AGENTIC TEXT-TO-SQL  - SMART DB AUTO-DETECTION")
    print("=" * 60)
    print(f"\nStatus: LLMs {'✅ Connected' if LLM_AVAILABLE else '⚠️  Using Fallback'}")
    
    print("\n📖 USAGE:")
    print("  1. interactive_demo()  # Run interactive shell")
    print("  2. run_agentic_pipeline(question='How many singers?')  # Auto-detect DB")
    print("  3. run_agentic_pipeline(db_id='concert_singer', question='How many singers?')  # Explicit DB")
    print("  4. db_discovery.search_databases('singer')  # Search for DBs")
    print("  5. db_discovery.get_best_matching_db('Which artists are from UK?')  # AI DB match")
    ...
    print("  • Smart DB suggestions when name is wrong")
    print("  • Auto-detect DB from question keywords")
    print("  • Interactive menu for manual or auto DB selection")